...
namespace LethalAPI.LibTerminal
{
    ...
    public class TerminalRegistry
    {
        ...
        {
            RegisterCommandInternal(command, command.Name);
            var aliases = command.Method.GetCustomAttribute<CommandAliasAttribute>();
            ...
            {
                RegisterCommandInternal(command, alias);
            }
        }

        /// <summary>
        /// Registers a command instance with the specified command name
        /// </summary>
        /// <param name="command">Command instance to register</param>
        /// <param name="name">Command name to register instance as</param>
        private static void RegisterCommandInternal(TerminalCommand command, string name)
        {
            List<TerminalCommand> commands;

            if (!m_RegisteredCommands.TryGetValue(name, out commands))
            {
                commands = new List<TerminalCommand>();
                m_RegisteredCommands[name] = commands;
            }
        ...
        {
            DeregisterInternal(command, command.Name);
            var aliases = command.Method.GetCustomAttribute<CommandAliasAttribute>();
            ...
            foreach (var alias in aliases.Aliases)
            {
                DeregisterInternal(command, alias);
            }
        }

        /// <summary>
        /// Deregisters a command instance, registered with the specific command name
        /// </summary>
        /// <param name="command">The command instance to deregister</param>
        /// <param name="name">The command name/alias to deregister from</param>
        private static void DeregisterInternal(TerminalCommand command, string name)
        {
