...
namespace LethalAPI.LibTerminal.Models
{
    ...
    public class TerminalCommand
    {
        ...
        /// <summary>
        /// The invoker to use to execute this command
        /// </summary>
        public ICommandInvoker? Invoker { get; }

        public TerminalCommand(string name, MethodInfo method, object instance, bool clearConsole, string? syntax = null, string? description = null, int priority = 0, ICommandInvoker? invoker = null)
        {
            Name = name;
            Method = method;
            Instance = instance;
        ...
        public static TerminalCommand FromMethod(MethodInfo info, object instance, string? overrideName = null)
        {
            ...
            var customInvoker = info.GetCustomAttribute<CommandInvokerAttribute>()
                ?? info.DeclaringType.GetCustomAttribute<CommandInvokerAttribute>();
            ...
            return new TerminalCommand(name, info, instance, clear, syntax, description, priority, customInvoker?.CreateInvoker());
        }
        ...
        public bool TryCreateInvoker(ArgumentStream arguments, ServiceCollection services, out Func<object?>? invoker)
        {
            if (Invoker != null)
            {
                if (Invoker.TryCreateInvoker(arguments, services, Method, out var customInvoker) && customInvoker != null)
                {
                    invoker = () => customInvoker(Instance);
                    return true;
                }
            }
            else if (CommandHandler.CurrentInterface != null && CommandHandler.CurrentInterface is ICommandInvoker custom)
            {
                if (custom.TryCreateInvoker(arguments, services, Method, out var interfaceInvoker) && interfaceInvoker != null)
                {
                    invoker = () => interfaceInvoker(Instance);
                    return true;
                }
            }
            else if (CommandActivator.TryCreateInvoker(arguments, services, Method, out var activatedInvoker) && activatedInvoker != null)
            {
