...
namespace LethalAPI.LibTerminal.Models
{
    ...
    public class ArgumentStream
    {
        ...
        /// <summary>
        /// Tries to read the next string as the specified type, without advancing the stream. Supports primitives, and some custom types defined in <seealso cref="StringConverter"/>
        /// </summary>
        /// <param name="type">The type to parse the string as</param>
        /// <param name="value">Resulting object instance</param>
        /// <returns><see langword="true"/> if the string could be parsed as the specified type</returns>
        public bool TryPeakNext(Type type, out object? value)
        {
            if (EndOfStream)
            {
                value = null;
        ...
        /// <param name="value">Resulting object instance</param>
        /// <returns><see langword="true"/> if the string could be parsed as the specified type</returns>
        public bool TryPeakNext<T>(out T value)
        {
            if (EndOfStream)
            {
                value = default!;
            ...
            if (StringConverter.TryConvert(Arguments[Index], typeof(T), out var parsedValue))
            {
                value = (T)parsedValue!;
                return true;
            }
        ...
        public bool TryReadRemaining(out string? result)
        {
            ...
            var padWithSpace = false;

            foreach (var argument in Arguments.Skip(Index))
            {
                if (padWithSpace && !argument.StartsWith("\n"))
                {
                    builder.Append(' ');
                }
                builder.Append(argument);

                padWithSpace = !argument.EndsWith("\n");
            }
