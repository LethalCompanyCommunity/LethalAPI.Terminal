// -----------------------------------------------------------------------
// <copyright file="ConfirmInteraction.cs" company="LethalAPI Modding Community">
...
{
    /// <summary>
    /// Initializes a new instance of the <see cref="ConfirmInteraction"/> class.
    ...
    /// Creates a confirmation interaction with the specified prompt.
    /// </summary>
    /// <param name="promptBuilder">Builder method to create a response <see cref="TerminalNode"/>, displayed as the response/prompt for the confirm/deny dialog.</param>
    /// <remarks>
    /// You do not need to add any custom Confirm/Deny text, as it is added by the interaction.
    /// </remarks>
    ...
    /// </para>
    /// <para>
    /// Services are injected into the confirm/deny handler parameters, from command context, and contested registered with <see cref="WithContext(object[])"/>.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Gets command response/interaction prompt to display to the user.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets delegate execution when the action is denied, or an ambiguous response is received.
    /// </summary>
    ...
    {
        this.Prompt = ScriptableObject.CreateInstance<TerminalNode>();
    ...
    /// <returns>Parent confirmation interaction.</returns>
    /// <remarks>
    /// Parameters are injected into the delegate, based from the command context, and contested provided with <see cref="WithContext(object[])"/>.
    /// </remarks>
    ...
    {
        this.Services.WithServices(services);
        return this;
    }
    ...
    {
        string text = this.Prompt.displayText ?? string.Empty;

        text = TextUtil.SetEndPadding(text, '\n', 2) + "Please CONFIRM or DENY.\n";

        this.Prompt.displayText = text;
    }
}